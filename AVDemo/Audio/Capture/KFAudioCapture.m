...
- (void)startRunning
{
    ...
    dispatch_async(_captureQueue, ^{
        if (!wealSelf.audioCaptureInstance) {
            NSError *error = nil;
            [wealSelf setupAudioCaptureInstance:&error];
            if (error) {
                [wealSelf callBackError:error];
                return;
            }
        }
        
        // 开始采集
        OSStatus startStatus = AudioOutputUnitStart(wealSelf.audioCaptureInstance);
        if (startStatus != noErr) {
            // 捕捉并回调开始采集时的错误。
            [wealSelf callBackError:[NSError errorWithDomain:NSStringFromClass([KFAudioCapture class]) code:startStatus userInfo:nil]];
        }
    });
}
...
- (void)stopRunning
{
    ...
    dispatch_async(_captureQueue, ^{
        if (wealSelf.audioCaptureInstance) {
            // 停止采集
            OSStatus stopStatus = AudioOutputUnitStop(wealSelf.audioCaptureInstance);
            if (stopStatus != noErr) {
                [wealSelf callBackError:[NSError errorWithDomain:NSStringFromClass([KFAudioCapture class]) code:stopStatus userInfo:nil]];
            }
        }
    });
}
