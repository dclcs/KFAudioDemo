...
@property (nonatomic, strong) AVAssetWriter *muxWriter;
...
- (instancetype)initWithConfig:(KFMuxerConfig *)config
{
    ...
    if (self) {
        _config = config;
        _muxerQueue = dispatch_queue_create("com.KFAudioDemo.muxerQueue", DISPATCH_QUEUE_SERIAL);
...
- (void)startWriting
{
    ...
    dispatch_async(self.muxerQueue, ^{
        ...
        [weakSelf _reset];
        weakSelf.muxerStatus = KFMP4MuxerStatusRunning;
...
- (void)cancelWriting
{
    ...
    dispatch_async(self.muxerQueue, ^{
        ...
        weakSelf.muxerStatus = KFMP4MuxerStatusCancelled;
        dispatch_semaphore_signal(weakSelf.semaphore);
    });
}
...
- (void)appendSampleBuffer:(CMSampleBufferRef)sampleBuffer
{
    ...
    dispatch_async(self.muxerQueue, ^{
        ...
        // 1. 添加数据到队列;
        [weakSelf _enqueuSampleBuffer:sampleBuffer];
...
- (void)stopWriting:(void (^)(BOOL, NSError * _Nonnull))completeHandler
{
    __weak typeof(self) weakSelf = self;
    dispatch_async(self.muxerQueue, ^{
        dispatch_semaphore_wait(weakSelf.semaphore, DISPATCH_TIME_FOREVER);
...
- (BOOL)_setupMuxWriter:(NSError **)error
{
    if (!self.config.outputURL) {
        *error = [self muxerErrorWithCode:40003 userInfo:nil];
        return NO;
    }
...
- (void)_enqueuSampleBuffer:(CMSampleBufferRef)sampleBuffer
{
    CFRetain(sampleBuffer);
...
- (void)_reset
{
    ...
    while (CMSimpleQueueGetCount(_videoQueue)) {
...
- (NSError *)muxerErrorWithCode:(NSInteger)code userInfo:(NSDictionary<NSErrorUserInfoKey, id> *)userInfo
{
    return [NSError errorWithDomain:NSStringFromClass([KFMP4Muxer class]) code:code userInfo:userInfo];
}
